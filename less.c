...
int main() {
    struct Contact contacts[] = {
        {"Михаил", "Синица", 18, 89529},
        {"Тимофей", "Синица", 7, 89293},
        {"Сергей", "Деним", 12, 89831},
        {"Андрей", "Куплинов", 88, 89050},
    };

    int size = 4;
    printf("Исходный справочник:\n");
    printContacts(contacts, size);
    printf("Ключ сортировки(0 -- Фамилия + Имя  1 -- Возраст + Номер): ");
    scanf("%d", &sortKey);
    printf("Направление сортировки(1 - по возрастанию 0 - по убыванию): ");
    scanf("%d", &ascending);
    if (sortKey==0 && ascending==1){
        InsertSort1(contacts,size);
    }
    if (sortKey==0 && ascending==0){
        InsertSort2(contacts,size);
    }
    if (sortKey==1 && ascending==1){
        InsertSort3(contacts,size);
    }
    if (sortKey==1 && ascending==0){
        InsertSort4(contacts,size);
    }
